...
export default async function NewUltrasound({ params }: { params: { babyId: string } }) {
  ...
  return (
    <PageShell max="3xl">
      <PageHeader backHref={`/babies/${params.babyId}/prenatal/ultrasounds`} backLabel="Ultrasounds"
        eyebrow="Add" eyebrowTint="brand" title="Ultrasound" />
      <Card><CardContent className="py-6">
        <UltrasoundForm babyId={params.babyId} />
      </CardContent></Card>
      ...
      <section className="rounded-2xl bg-white border border-slate-200 shadow-card overflow-hidden">
        <div className="px-5 py-3 border-b border-slate-100 flex items-center gap-2">
          ...
          <h3 className="text-sm font-bold text-ink-strong">Attach the original scan</h3>
