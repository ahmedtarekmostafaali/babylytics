...
export default async function UltrasoundsList({
  ...
  const groups = Array.from(buckets.entries()).map(([k, list]) => ({
    k, heading: groupHeading(list[0]!.scanned_at, t), list,
  }));
  ...
  return (
    <PageShell max="5xl">
      <PageHeader backHref={`/babies/${params.babyId}`} backLabel={t('prenatal.back_label')}
        eyebrow={t('prenatal.eyebrow')} eyebrowTint="brand"
        title={t('prenatal.us_title')}
        subtitle={t('prenatal.us_subtitle')}
        right={
          perms.canWriteLogs ? (
            <div className="flex items-center gap-2">
              <BulkDelete babyId={params.babyId} table="ultrasounds" timeColumn="scanned_at"
                visibleIds={rows.map(r => r.id)} kindLabel={t('prenatal.us_title').toLowerCase()} />
              <Link href={`/babies/${params.babyId}/prenatal/ultrasounds/new`}
                ...
                <Plus className="h-4 w-4" /> {t('prenatal.us_add')}
              </Link>
            </div>
          ) : (
            ...
            <section key={g.k}>
              ...
              <ul className="divide-y divide-slate-100">
                {g.list.map(u => {
                  ...
                  const bits: string[] = [];
                  if (u.efw_g != null) bits.push(`EFW ${u.efw_g}${t('prenatal.us_efw_g')}`);
                  if (u.fhr_bpm != null) bits.push(`${u.fhr_bpm} ${t('prenatal.us_bpm')}`);
                    ...
                    <li key={u.id}>
                      <Link href={`/babies/${params.babyId}/prenatal/ultrasounds?range=${range.key}&id=${u.id}`}
                        ...
                        <div className="min-w-0">
                          <div className="font-semibold text-ink-strong truncate">{t('prenatal.us_label')}</div>
        ...
        <div className="space-y-4 lg:sticky lg:top-4 self-start">
          <section className="rounded-2xl bg-white border border-slate-200 shadow-card">
            <div className="flex items-center justify-between px-5 py-3 border-b border-slate-100">
              <h3 className="text-sm font-bold text-ink-strong">{t('prenatal.us_details')}</h3>
                    ...
                    <>
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="font-bold text-ink-strong">{t('prenatal.us_full_label')}</div>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {selected.efw_g != null && (
                          <div className="rounded-xl bg-brand-50 border border-brand-100 px-3 py-2">
                            <div className="text-[10px] uppercase tracking-wider text-brand-700 font-semibold">{t('prenatal.us_efw')}</div>
                            <div className="text-xl font-bold text-ink-strong leading-tight">{selected.efw_g}<span className="text-xs text-ink-muted ml-1 font-normal">{t('prenatal.us_efw_g')}</span></div>
                          </div>
                        )}
                        {selected.fhr_bpm != null && (
                          <div className="rounded-xl bg-coral-50 border border-coral-100 px-3 py-2">
                            <div className="text-[10px] uppercase tracking-wider text-coral-700 font-semibold">{t('prenatal.us_fhr')}</div>
                            <div className="text-xl font-bold text-ink-strong leading-tight">{selected.fhr_bpm}<span className="text-xs text-ink-muted ml-1 font-normal">{t('prenatal.us_bpm')}</span></div>
                          </div>
                        )}
                        {selected.sex_predicted && selected.sex_predicted !== 'undetermined' && (
                          <div className="rounded-xl bg-lavender-50 border border-lavender-100 px-3 py-2">
                            <div className="text-[10px] uppercase tracking-wider text-lavender-700 font-semibold">{t('prenatal.us_sex')}</div>
                      ...
                      {selected.summary && (
                        <div>
                          <div className="text-[10px] uppercase tracking-wider text-ink-muted font-semibold">{t('prenatal.us_summary')}</div>
                      ...
                      {selected.anomalies && (
                        <div className="rounded-xl bg-coral-50 border border-coral-200 p-3">
                          <div className="text-[10px] uppercase tracking-wider text-coral-700 font-semibold flex items-center gap-1">
                            <AlertTriangle className="h-3 w-3" /> {t('prenatal.us_anomalies')}
                          </div>
      ...
      <Comments babyId={params.babyId} target="babies" targetId={params.babyId}
        pageScope="prenatal_ultrasounds_list" title={t('page.page_comments')} />
    </PageShell>
  );
}
