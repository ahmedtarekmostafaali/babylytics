  ...
  return (
    <PageShell max="5xl">
      <PageHeader backHref={`/babies/${params.babyId}`} backLabel="Overview"
        eyebrow="Track" eyebrowTint="mint"
        title="Activities"
        subtitle="All recorded activity sessions."
        right={
          perms.canWriteLogs ? (
            <div className="flex items-center gap-2">
              <BulkDelete babyId={params.babyId} table="activity_logs" timeColumn="started_at"
                visibleIds={rows.map(r => r.id)} kindLabel="activities" />
      ...
      <Comments babyId={params.babyId} target="babies" targetId={params.babyId}
        pageScope="activities_list" title="Page comments" />
    </PageShell>
  );
}
