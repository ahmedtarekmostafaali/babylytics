  ...
  return (
    <PageShell max="5xl">
      <PageHeader backHref={`/babies/${params.babyId}`} backLabel={baby.name}
        ...
        title={t('trackers.sleep_title')}
        subtitle={t('trackers.sleep_sub')}
        right={
          perms.canWriteLogs ? (
            <div className="flex items-center gap-2">
              <BulkDelete babyId={params.babyId} table="sleep_logs" timeColumn="start_at"
                visibleIds={rows.map(r => r.id)} kindLabel={t('trackers.sleep_title').toLowerCase()} />
              <Link href={`/babies/${params.babyId}/sleep/new`}
                ...
                <Plus className="h-4 w-4" /> {t('trackers.sleep_cta')}
              </Link>
            </div>
          ) : (
      ...
      <Comments babyId={params.babyId} target="babies" targetId={params.babyId}
        pageScope="sleep_list" title={t('page.page_comments')} />
    </PageShell>
  );
}
